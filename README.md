...
在左侧工作流导航树中选中 Build OpenWrt。

点击右侧出现的 Run workflow 手动执行编译任务。

云端实例分配与交叉编译耗时约 2 小时。任务标记为绿色成功状态后，点击进入该条目的详情页面。

滚动至页面最下方的 Artifacts 区域，点击下载生成的固件压缩包。升级时请使用包内以 sysupgrade.itb 结尾的文件。

模块化定制指南
业务层组件与底层架构已实现物理隔离。如需调整编译预装的插件，无需介入复杂的源码层配置文件：

编辑 .github/workflows/build-openwrt.yml 文件。
定位至顶层 env 声明区域。
修改 CUSTOM_PACKAGES 变量的值。填入所需的 luci-app- 或 kmod- 模块名称，各项之间以单个空格分隔。
保存提交后，下一次触发的编译将自动集成对应组件。
